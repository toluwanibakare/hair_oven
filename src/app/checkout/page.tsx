        ...
        {items.length === 0 ? (
          ...
          <div className="mt-8 grid lg:grid-cols-12 gap-8">
            <div className="lg:col-span-7 bg-white border border-[rgba(28,18,14,0.08)] p-6 space-y-4">
              <h2 className="text-[11px] tracking-[0.16em] uppercase font-bold">{t.checkout.contact}</h2>
              ...
              <h2 className="text-[11px] tracking-[0.16em] uppercase pt-4 font-bold">{t.checkout.shipping}</h2>
              ...
              <button className="w-full h-12 bg-[#2B1B12] text-white text-[11px] tracking-[0.16em] uppercase font-bold hover:bg-[#B8860B] transition-colors">{t.checkout.payPrefix} {formatPrice(cartTotal)} - {t.checkout.paySuffix}</button>
            ...
            <div className="lg:col-span-5 bg-[#FDF8F0] border border-[rgba(28,18,14,0.06)] p-6 h-fit">
              <h2 className="text-[11px] tracking-[0.16em] uppercase font-bold">{t.checkout.summary}</h2>
                ...
                {items.map((i) => (
                  <div key={i.product.id} className="flex gap-3 text-sm">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm font-bold text-[#2B1B12] mt-1">{formatPrice(i.product.price * i.qty)}</div>
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-6 h-px bg-[rgba(28,18,14,0.08)]" />
              <div className="mt-4 flex justify-between text-sm"><span>{t.checkout.subtotal}</span><span className="font-bold text-[#2B1B12]">{formatPrice(cartTotal)}</span></div>
